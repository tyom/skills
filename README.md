...
- `flow` — Traces a flow through its real source — a request lifecycle, pipeline, state machine, or user journey — and renders it as an interactive graph with a detail panel per step, one or more flows as tabs. Slash-only (`/flow`).
