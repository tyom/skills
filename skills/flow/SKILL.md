...
The build measures each flow rather than the window it opens in, and prints two numbers for every one. V(G) counts the routes through the flow. The rank count is its longest path. A flow the build reports as deep is usually answering two questions. It names the node every path already crosses and the depth of each half. Split there, and the seam is found rather than invented. It also names the nodes on that path that take one edge in, one out, and carry no label, because each costs a rank and earns none. Branching is a property of the subject and no split reduces it, so a high V(G) alone is reported and left alone.
...
The fit holds a zoom floor rather than shrinking without limit, so a flow bigger than the window opens cropped. That is a fault when the reader wanted an overview and expected when they asked for detail. For an overview, narrow the question, split at the seam the build named, or make one route the subject. For detail, say in the summary that the page opens cropped, and that L fits the whole graph, M shows the minimap, and the arrow keys walk it. Dropping a level does not help either way, because a level changes a node's width, not the flow's depth.
